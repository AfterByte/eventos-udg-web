...
export default class TaskBar extends Component<{},state> {
  ...
  deleteTag = (e:any, tag:string) => {
    ...
    this.setState(state => ({
      tags: [...this.state.tags.filter(tagS => stringID !== this.state.tags.indexOf(tagS))],
    }))
  }
      ...
      <div className="col-start-2 col-end-7 w-full">
        <div className="grid xl:grid-cols-4 grid-cols-4 shadow-md text-center w-full md:w-full h-auto items-center py-2 sm:py-5 bg-white mt-0 sm:mt-2">
          <div className="col-span-1 grid grid-cols-1 sm:grid-cols-5 md:text-left ml-2 sm:ml-4 relative">
            <button onClick={() => this.handleClick()} className="px-2 py-2 col-span-2 text-left font-normal text-white bg-blue-500 rounded-md items-center flex justify-between">
              <p className="hidden sm:grid sm:text-md md:text-sm lg:text-sm xl:text-lg">Ordenar por</p>
              <img className="h-6" style={{filter: "invert(100%)"}} src={Sort} alt={"SortIcon"}/>
            </button>
            {this.state.isOpen ? 
            <div className="grid grid-cols-5 absolute w-auto text-center mt-10 xl:mt-16 z-10">
              <div className="col-span-5 sm:col-span-2 bg-white border">
                <a className="border block px-2 py-2 hover:bg-blue-500 hover:text-white text-xs font-semibold sm:text-base sm:font-normal">Fecha (Cercanos)</a>
                <a className="border block px-2 py-2 hover:bg-blue-500 hover:text-white text-xs font-semibold sm:text-base sm:font-normal">Fecha (Lejanos)</a>
                <a className="border block px-2 py-2 hover:bg-blue-500 hover:text-white text-xs font-semibold sm:text-base sm:font-normal">Cupo (Mas llenos)</a>
                <a className="border block px-2 py-2 hover:bg-blue-500 hover:text-white text-xs font-semibold sm:text-base sm:font-normal">Cupo (Menos llenos)</a>
              </div>
            </div> : <div></div>}
          </div>
          <div className="col-span-3 sm:col-span-3 grid grid-cols-1 sm:grid-cols-1 xl:grid-cols-6">
            <div className="hidden col-span-1 py-1 ml-2 sm:ml-0 sm:pb-0 sm:col-span-4  lg:flex justify-end">
              {this.state.tags.map(tag => (
                <span className={this.state.showTag 
                  ...
                  <button onClick={(e) => this.deleteTag(e, tag)}><img className="h-4 ml-1" src={Clear} alt={"ClearTagIcon"}/></button>
                </span>
              ))}
            </div>
            <div className="col-span-1 sm:col-span-2 flex ml-2 sm:ml-0 justify-center sm:justify-end">
              <form className="flex w-full" onSubmit={(e) => e.preventDefault()}>
                <label className="flex w-full h-full mr-2 sm:mr-0 border border-gray-500 border-opacity-50">
                  ...
                  <input onChange={(e) => this.collectSearchData(e)} value={this.state.tags.toString()} className="sm:mr-0 px-1 bg-gray-500 bg-opacity-25 w-full" type="text" name="buscar"/>
                </label>
                <div className="hidden sm:grid mr-2 px-6 py-2 text-md font-normal text-white bg-blue-500 items-center">Buscar</div>
              </form>
            </div>
          </div>
        </div>
